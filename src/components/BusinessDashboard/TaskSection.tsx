    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {tasks.map((task) => (
          ...
          >
            <div>
              <h4 className={`font-bold text-sm ${task.textColor}`}>
                {task.title}
              </h4>
              <p className="text-xs text-gray-500 mt-1">{task.subtitle}</p>
            </div>
            <button
              className={`${task.buttonBg} text-white w-full py-2 rounded-lg text-xs font-semibold hover:opacity-80 transition-opacity`}
            >
              {task.buttonText}
            </button>
          </div>
